// edid.go
// This file is copied from https://github.com/anoopengineer/edidparser.
// This file adds the ability to modify some basic attributes of the EDID on top of the original file.
...
func (e *Edid) CalcSum() byte {
	var checkSum byte = 0
	for i := 0; i < 127; i++ {
		checkSum += e.originBytes[i]
	}
...
func (e *Edid) SetManufacturerId(mid string) error {
	...
	if err != nil || !match {
		return fmt.Errorf("Invalid Manufacturer ID")
	}
	e.ManufacturerId = mid
	tmp := reverseManufacturerId(mid)
	e.originBytes[8] = tmp[0]
	e.originBytes[9] = tmp[1]
	return nil
}

func (e *Edid) SetProductCode(pcode uint16) error {
	e.ProductCode = pcode
	buf := NewByteSlice(e.originBytes)
	if _, err := buf.Seek(10, 0); err != nil {
		return fmt.Errorf("Error seeking to product code:%v", err)
	}
	return binary.Write(buf, binary.LittleEndian, pcode)
}

func (e *Edid) SetSerialNumber(snum uint32) error {
	e.SerialNumber = snum
	buf := NewByteSlice(e.originBytes)
	if _, err := buf.Seek(12, 0); err != nil {
		return fmt.Errorf("Error seeking to target:%v", err)
	}
	return binary.Write(buf, binary.LittleEndian, snum)
}

func (e *Edid) SetWeekOfManufacture(week uint8) error {
	if week > 54 && week != 255 {
		return fmt.Errorf("Invalid week of manufacture")
	}
	if week == 255 {
		week = 16
	}
	e.originBytes[16] = week
	return nil
}

func (e *Edid) SetYearOfManufacture(year int) error {
	if year < 1990 {
		return fmt.Errorf("Invalid year of manufacture")
	}
	e.originBytes[17] = uint8(year - 1990)
	return nil
}

func (e *Edid) SetMonitorName(name string) error {
	if len(name) > 13 {
		return fmt.Errorf("Monitor name too long")
	}
	for i := 0; i < 4; i++ {
		temp := e.originBytes[54+i*18 : 54+(i+1)*18]
		if ((uint32(temp[1])<<8)|uint32(temp[0]))*10 != 0 {
			continue
		}
		if temp[3] != 0xFC {
			continue
		...
		for i := 0; i < 13; i++ {
			if i < len(name) {
				if name[i] > 0x7F {
					return fmt.Errorf("Monitor name contains non-ASCII characters")
				}
				temp[5+i] = byte(name[i])
			} else if i == len(name) {
				temp[5+i] = 0x0A
			} else {
				temp[5+i] = 0x20
		...
		buf := NewByteSlice(e.originBytes)
		_, err := buf.Seek(54+i*18, 0)
		if err != nil {
			return fmt.Errorf("Error seeking to monitor name:%v", err)
		}
		_, err = buf.Write(temp)
		return err
	}
	return fmt.Errorf("No free monitor name descriptor found")
}

func reverseManufacturerId(s string) [2]byte {
	var manufacturerId [2]byte

	manufacturerId[0] = ((s[0] - 'A' + 1) << 2) | ((s[1] - 'A' + 1) >> 3)
	manufacturerId[1] = (((s[1] - 'A' + 1) & 0x07) << 5) | (s[2] - 'A' + 1)

	return manufacturerId
}

func (e *Edid) ToBytes() []byte {
	e.SetSum()
